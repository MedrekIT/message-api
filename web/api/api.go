...
func Routes(apiCfg *ApiConfig) http.Handler {
	...
	mu.HandleFunc("POST /api/groups/{groupID}/members", apiCfg.joinGroupHandler)
	mu.HandleFunc("POST /api/groups/{groupID}/invitations", apiCfg.createInvitationHandler)
	mu.HandleFunc("POST /api/friendships/{senderID}", apiCfg.acceptFriendHandler)
	mu.HandleFunc("POST /api/friendships/requests/{receiverID}", apiCfg.requestFriendHandler)
